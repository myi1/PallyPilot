...
AD.NODES = {
  ...
  -- TIER 5 -- FARM SURVIVAL (rebuilt every prestige). WIPED on prestige, so this
  -- is what you REBUY each reclimb to survive AoE pack farming. Ordered
  -- AoE-sustain first (`farm = true` = the pack-farming picks the advisor leads
  -- with once your perms are already owned). Node effects verified from the
  -- client TalentDatabase (tree.json), 2026-08-29.
  { key = "splashguard", name = "Splashguard", tier = 5, survival = true, farm = true,
    ids = { 328, 329, 330 }, costs = { 1000, 2500, 7500 },
    effect = "Reduces Area-of-Effect damage taken per rank -- THE pack-farming node, and cheap. Rebuy first each prestige." },
  { key = "undyingspark", name = "Undying Spark", tier = 5, survival = true, farm = true,
    ids = { 347, 348, 349, 531, 850, 910 },
    costs = { 5000, 15000, 50000, 500000, 750000, 1000000 },
    effect = "In Hardcore, converts to FLAT all-damage reduction per rank -- the biggest survival lever vs HC packs. Rebought each prestige." },
  { key = "refusedrequiem", name = "Refused Requiem", tier = 5, survival = true, farm = true,
    ids = { 340, 341, 628, 848, 909 },
    costs = { 25000, 25000, 250000, 500000, 750000 },
    effect = "In Hardcore, FLAT all-damage reduction per rank (stacks with Undying Spark). Rebought each prestige." },
  { key = "victoryfeast", name = "Victory Feast", tier = 5, survival = true, farm = true,
    ids = { 388 }, costs = { 7500, 15000, 25000 },
    effect = "Restore % max HP on every XP/honor kill -- heal-on-kill sustain that scales with your pack-clear speed." },
  { key = "secondwind", name = "Second Wind", tier = 5, survival = true, farm = true,
    ids = { 391 }, costs = { 1500, 3000, 5000 },
    effect = "Restore % max HP out of combat -- between-pull recovery." },
  { key = "vitalinfusion", name = "Vital Infusion", tier = 5, survival = true, farm = true,
    ids = { 395 }, costs = { 1500, 3000, 5000, 10000, 25000, 250000 },
    effect = "Potions restore an extra % of max HP -- burst recovery in a bad pull." },
  { key = "borrowedtime", name = "Borrowed Time", tier = 5, survival = true,
    ids = { 335, 336, 337, 338, 339 },
    costs = { 5000, 10000, 15000, 20000, 25000 },
    effect = "Cheat Death: survive a killing blow -- safety net, rebought each prestige." },
...
AD.TIER_NAMES = {
  ...
  [3] = "Permanent stats",
  [4] = "Quality of life",
  [5] = "Farm survival (rebuilt each prestige)",
}
